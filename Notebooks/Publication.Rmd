...
```{r}
ps_CSS = normalize(ps_bact_filtered, method="CSS")
```
...
```{r, fig.width=9, fig.height=5, text.width=10}
alpha_indexes_bact <- estimate_richness(ps_scaffolds_filtered, split = TRUE, c("Shannon", "Simpson", "InvSimpson"))

kruskal.test(alpha_indexes_bact$Shannon ~ sample_data(ps_scaffolds_filtered)$City)
kruskal.test(alpha_indexes_bact$InvSimpson ~ sample_data(ps_scaffolds_filtered)$City)
...
```{r}
plot_richness(ps_bact_filtered, x = "Regional_Hospital",
              measures=c("Shannon", "Simpson"), nrow = 2, sortby = "Shannon") + 
              geom_boxplot() + geom_point(size=1, alpha=0.5)
plot_richness(ps_bact_filtered, x = "Hospital_Type",
              measures=c("Shannon", "Simpson"), nrow = 2, sortby = "Shannon") + 
              geom_boxplot() + geom_point(size=1, alpha=0.5)
```

```{r}
kruskal.test(alpha_indexes_bact$Shannon ~ sample_data(ps_bact_filtered)$Regional_Hospital)
...
```
```{r}
pairwise.wilcox.test(alpha_indexes_bact$Shannon,sample_data(ps_bact_filtered)$Hospital_Type,
                     p.adjust.method = "BH")
```
