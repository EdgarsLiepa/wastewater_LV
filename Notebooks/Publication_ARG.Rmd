...
```{r}
knitr::opts_chunk$set(dev = 'cairo_pdf', dpi = 300)
```
...
metadata <- read.csv(METADATA_PATH, header = TRUE, 
                     colClasses = c(Date = "character", 
                                    Dairy_farming="character", 
                                    Meat_production="character", 
                                    Metal_processing="character", 
                                    Washrooms="character"
                                    )
                     )
...
and sequencing read overlap was counted in positions with ht-seq. 

Combined results from HT-Seq is imported in R as a gene abundance matrix
and combined with metadta information in **Phyloseq** object
...
Display count of AMR gene reads found in samples.

```{r, plot_amr, warning=FALSE}

plot_bar(ps_amr)

options(repr.plot.width=4, repr.plot.height=4)
hist(log10(sample_sums(ps_amr)), 
     breaks=50, 
     main="Sample size distribution", 
     xlab="Sample size (log10)", 
     ylab="Frequency", col="#007c80")

```
### Filtering 

Since we removed some samples from bacterial analysis, we have to remove them also here.
...
hist(log10(sample_sums(ps_amr_good)), 
     breaks=50, 
     main="Sample size distribution", 
     xlab="Sample size (log10)", 
     ylab="Frequency", 
...
Now we remove singleton reads that are assigned to genes with only one aligned read across all samples.
...
Range of sample size in reconstructed samples.

```{r, amr_depth}
boxplot(sample_sums(ps_scaffolds_filtered), 
        main="Samples depth across samples removed singletons", 
        xlab="", ylab="Number of reads", 
        col="#a6093d")

text(
  y=boxplot.stats(sample_sums(ps_scaffolds_filtered))$stats,
  labels=boxplot.stats(sample_sums(ps_scaffolds_filtered))$stats, 
  x=1.25
  )
```

The dimensions of the filtered data
...
Total sequence count after filtering
...
AMR gene distribution across samples

```{r, abundance_plot, warning=FALSE}
ps_amr_good %>%
    ps_mutate(Group = factor(sample_data(ps_amr_good)$City)) %>%
    tax_agg("ARG") %>%
    ps_seriate(dist = "bray", method = "OLO_ward") %>%
    comp_barplot(tax_level = "ARG", 
                 sample_order = rownames(sample_data(ps_amr_good)[order(sample_data(ps_amr_good)$Sample), ]), 
                 n_taxa = 10, 
                 label = "Sample") +
...
Load extra functions to parse CARD DB and reconstructed ARGs.
...
```{r, family_pl, fig.width=10, fig.height=4.5, warning=FALSE}
physeq_amr_families %>%
...
```{r, combined_pl, fig.height=12, fig.width=14, warning=FALSE}
...
Show all gene distribution on a NMDS plot with top10 genes colored
...
Here we evaluate how sample size of reconstructed reads impacts the recall of ARGs using normalization by subsampling (rarefaction)

Display rarefaction curves.

```{r, rare_curve_amr, warning=FALSE}
tab <- otu_table(ps_amr_good)
class(tab) <- "matrix"
tab <- t(tab)
...
Show smallest sample size to rarefy to and do rarefication if needed.

```{r, rare_amr}
# find minimal deapth to rarefy to
df=as.data.frame(sample_sums(ps_amr_good))
...
Since individual sample curves showed good recall and samples sizes are not too far apart this was not used.



For beta diversity analysis normalization by cumulative sum scaling (CSS) was done to phyloseq object with singleton removed reads.

```{r, css_amr, results='hide'}
ps_amr_CSS = microbiomeMarker::normalize(ps_scaffolds_filtered, method="CSS")
```

## Most abundant genes

Show Top 15 most abundant genes and their relative count
...
top_genes <- sort(mean_percentage, decreasing = TRUE) %>% 
  head(n = 15) %>% 
  as.data.frame %>% 
  rownames
...
genes_transformed  = transform_sample_counts(physeq_drug_class, function(x) x / sum(x) )
...
genes_transformed  = transform_sample_counts(physeq_amr_families, function(x) x / sum(x) )
...
```{r, d_clas_proc}
genes_transformed  = transform_sample_counts(physeq_amr_categories, function(x) x / sum(x) )
...
alph_city <- plot_richness(ps_amr_good, x="City", 
...
alpha_indexes_amr <- estimate_richness(ps_amr_good, 
                                       split = TRUE, 
                                       c("Shannon", "Simpson", "InvSimpson"))

kruskal.test(alpha_indexes_amr$Shannon ~ sample_data(ps_amr_good)$City)
kruskal.test(alpha_indexes_amr$InvSimpson ~ sample_data(ps_amr_good)$City)
...
Lets show minimal and maximum diversity values.
...
pinpoint which specific means are significant different from the others
...
Lets compare how similar municipalities are between each other using bray-curtis distance on a NMDS plot.

```{r, beta_amr_city}
GP.ord2 <- ordinate(ps_amr_CSS, "NMDS", "bray")
p1 = plot_ordination(ps_amr_CSS, GP.ord2, 
                     type="samples", 
                     color = "City", 
                     title="City bray-distance NMDS")+
...
anosim(tax_bray_amr, sample_data(ps_amr_CSS)$City, 
       distance = "bray", 
       permutations = 9999)
...
Define SIAMCAT analysis function
...
Show differentialy abundant genes in a plot

```{r, assoc_hosp}
label_case_pairs <- list(
  list(label = "City", case = "Cesis"),
  list(label = "City", case = "Dobele"),
  list(label = "City", case = "Jelgava"),
  list(label = "City", case = "Jurmala"),
  list(label = "City", case = "Kuldiga"),
  list(label = "City", case = "Liepāja"),
  list(label = "City", case = "Madona"),
  list(label = "City", case = "Salaspils"),
  list(label = "City", case = "Saldus"),
  list(label = "City", case = "Sigulda"),
  list(label = "City", case = "Smiltene"),
  list(label = "City", case = "Talsi"),
  list(label = "City", case = "Tukums"),
  list(label = "City", case = "Valmiera"),
  list(label = "City", case = "Ventspils")
)
...
Now look if population size impacts AMR.

```{r, kruskal_popul}

kruskal.test(alpha_indexes_amr$Shannon ~ sample_data(ps_amr_good)$Population)
kruskal.test(alpha_indexes_amr$InvSimpson ~ sample_data(ps_amr_good)$Population)

```

```{r, div_popul}
plot_richness(ps_amr_good, x="Population", title="AMR diversity", measures=c("InvSimpson", "Shannon"), nrow = 2, sortby = "InvSimpson") + 
    geom_boxplot() + 
    theme_bw() +
    theme(legend.position = "none", axis.text.x = element_text(angle = 90, hjust = 1))+
  theme(
...
```{r, plot_beta_pop}
beta <- betadisper(tax_bray_amr, group=sample_data(ps_amr_CSS)$Population, type = "centroid")
print("BETADISPER")
print("Population")
print(anova(beta))
...
adonis2_rez<-adonis2(tax_bray_amr ~ as.factor(sample_data(ps_amr_CSS)$Population), data=tax_bray_amr, permutations = 9999)
print(adonis2_rez)
...
alpha_beta_arg <- plot_grid(
    alph_city, beta_plot,
    labels = c("a", "b"),
    label_size = 16,
     rel_widths = c(1, 1.5)
  ) + draw_figure_label(
      label=paste('PERMANOVA p = ',round(adonis2_rez$`Pr(>F)`[1], 3), " R2 = ", round(adonis2_rez$R2[1], 3)),
      position = "top.right") +
  draw_figure_label(
      label=paste('BETADISP p = ',),
      position = "top.right")
```
...
# Save as PNG
ggsave("../plots/Figure_6.jpg", plot = alpha_beta_arg, width = 16, height = 8, units = "in", dpi = 1200)

# Save as SVG
ggsave("../plots/Figure_6.tiff", plot = alpha_beta_arg, width = 16, height = 8, units = "in", dpi = 1200)
```
...
```{r, wilconx_hosp}
wilcox.test(alpha_indexes_amr$Shannon~sample_data(ps_amr_good)$Regional_Hospital,
                     p.adjust.method = "BH")

wilcox.test(alpha_indexes_amr$InvSimpson~sample_data(ps_amr_good)$Regional_Hospital,
                     p.adjust.method = "BH")

```


Now we calculate if Hospital Type has an effect. 


```{r, kurskal_amr}
kruskal.test(alpha_indexes_amr$Shannon ~ sample_data(ps_amr_good)$Hospital_Type)
kruskal.test(alpha_indexes_amr$InvSimpson ~ sample_data(ps_amr_good)$Hospital_Type)
```

Drug Class

```{r, wilconx_hosp_class}
alpha_indexes_dclass <- estimate_richness(physeq_drug_class, 
                                          split = TRUE, 
                                          c("Shannon", "Simpson", "InvSimpson"))

kruskal.test(alpha_indexes_dclass$Shannon~sample_data(physeq_drug_class)$Hospital_Type)

kruskal.test(alpha_indexes_dclass$InvSimpson~sample_data(physeq_drug_class)$Hospital_Type)
```

AMR family

```{r, wilconx_hosp_family}
alpha_indexes_amr_family <- estimate_richness(physeq_amr_families, 
                                              split = TRUE, 
                                              c("Shannon", "Simpson", "InvSimpson"))

kruskal.test(alpha_indexes_amr_family$Shannon~sample_data(physeq_amr_families)$Hospital_Type)

kruskal.test(alpha_indexes_amr_family$InvSimpson~sample_data(physeq_amr_families)$Hospital_Type)
```


```{r, richness_h_type, echo=FALSE}
# used for generating plots
# r1 <- plot_richness(ps_amr_good, x = "Hospital_Type",
...
```{r, beta_h_type}
GP.ord2 <- ordinate(physeq_amr_families, "NMDS", "bray")
p1 = plot_ordination(physeq_amr_families, 
                     GP.ord2, type="samples", 
                     color = "Hospital_Type",
                     title="Hospital_Type bray-distance NMDS")+
...
```{r, dune_h_typ_dun}
dunn_results <- dunn.test(alpha_indexes_amr$Shannon, 
          sample_data(ps_amr_good)$Hospital_Type, 
          method="bh")
```
...
Here are most differentially abundant genes
...
```{r, kruskal}
# remove NA values
ps_amr_ww_impact = subset_samples(ps_amr_good, 
                    sample_data(ps_amr_good)$Industrial_wastewater_impact != "None")


alpha_indexes_ww_impact <- estimate_richness(ps_amr_ww_impact, 
                                             split = TRUE, 
                                             c("Shannon", "Simpson", "InvSimpson"))
...
```{r,kruskal_food}
# remove NA values
ps_amr_ww_impact = subset_samples(ps_amr_good, sample_data(ps_amr_good)$Industrial_wastewater_impact_from_food != "None")
...
Municipalities were classified by Dairy_farming, Meat_production, Metal_processing and car washing facilities connected to wastewater system. 


```{r, wilconx_bin}
wilcox.test(alpha_indexes_amr$Shannon~sample_data(ps_scaffolds_filtered)$Dairy_farming,
            p.adjust.method = "BH")

wilcox.test(alpha_indexes_amr$Shannon~sample_data(ps_scaffolds_filtered)$Meat_production,
            p.adjust.method = "BH")

wilcox.test(alpha_indexes_amr$Shannon~sample_data(ps_scaffolds_filtered)$Metal_processing,
            p.adjust.method = "BH")

wilcox.test(alpha_indexes_amr$Shannon~sample_data(ps_scaffolds_filtered)$Washrooms, 
            p.adjust.method = "BH")
```
```{r, wilcox_sipm}
wilcox.test(alpha_indexes_amr$Simpson~sample_data(ps_scaffolds_filtered)$Dairy_farming,
            p.adjust.method = "BH")

wilcox.test(alpha_indexes_amr$Simpson~sample_data(ps_scaffolds_filtered)$Meat_production,
            p.adjust.method = "BH")

wilcox.test(alpha_indexes_amr$Simpson~sample_data(ps_scaffolds_filtered)$Metal_processing,
            p.adjust.method = "BH")

wilcox.test(alpha_indexes_amr$InvSimpson~sample_data(ps_scaffolds_filtered)$Washrooms, 
            p.adjust.method = "BH")
```
...
# remove NAs
ps_amr_ww_impact = subset_samples(ps_amr_CSS, 
                              sample_data(ps_amr_CSS)$Industrial_wastewater_impact != "None")
...
# remove NAs
ps_amr_ww_impact = subset_samples(ps_amr_CSS,
                  sample_data(ps_amr_CSS)$Industrial_wastewater_impact_from_food != "None")
...
metadata_col = colnames(sample_data(ps_amr_CSS))[9:13]
...
Show OTUs that are in 70% of the samples with abundance larger than and more than 0.001% relative abundance

```{r, core}

ps_amr_good@tax_table[microbiome::core_members(ps_amr_good, detection = 1 , prevalence = 70/100)]

```

Find core genes in cities with regional hospitals

```{r, core_genes2}

# Filter for hospital samples
arg_hospital <- subset_samples(ps_amr_good, Regional_Hospital == "yes")

# Filter for non-hospital samples
arg_nonhospital <- subset_samples(ps_amr_good, Regional_Hospital == "no")

core_arg_hospital <- microbiome::core(arg_hospital, detection=1, prevalence=70/100)

# Core in non-hospital samples
core_arg_nonhospital <- microbiome::core(arg_nonhospital, detection=1, prevalence=70/100)

```


Find non overlapping genes between cities with and without regional hospital 

```{r, core_genes3}
# Extract core feature names
hospital_core_genes <- taxa_names(core_arg_hospital)
nonhospital_core_genes <- taxa_names(core_arg_nonhospital)

# Compare (intersect, union, setdiff)
shared_cores <- intersect(hospital_core_genes, nonhospital_core_genes)

unique_hospital <- setdiff(hospital_core_genes, nonhospital_core_genes)
unique_nonhospital <- setdiff(nonhospital_core_genes, hospital_core_genes)

```


Intersection of genes shared in cities with and without regional hospitals

```{r, shared_arg}
as.data.frame(arg_hospital@tax_table[shared_cores][,"Tax"])
```

genes in cities with regional hospitals that are unique to group

```{r, hospital_arg}
as.data.frame(arg_hospital@tax_table[unique_hospital][,"Tax"])
```

genes in cities without regional hospitals that are unique to group

```{r, nonhospital_arg}
as.data.frame(arg_hospital@tax_table[unique_nonhospital][,"Tax"])
```
