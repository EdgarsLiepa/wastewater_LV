...
xMETADATA_PATH =  "../sampleMetadata.csv"
...
```{r, amr_depth}
boxplot(sample_sums(ps_scaffolds_filtered), main="Samples depth across samples removed singletons", xlab="", ylab="Number of reads", col="#a6093d")
...
alph_city <- plot_richness(ps_amr_good, x="City", measures=c("InvSimpson", "Shannon"), nrow = 2, sortby = "Shannon") + 
    geom_boxplot() + 
    theme_bw() +
...
alpha_indexes_amr <- estimate_richness(ps_amr_good, split = TRUE, c("Shannon", "Simpson", "InvSimpson"))
...
Show differential abundat genes in a plotte
...
Now look whether population size impact AMR.
...
```{r, wilconx_hosp}
alpha_indexes_dclass <- estimate_richness(physeq_drug_class, split = TRUE, c("Shannon", "Simpson", "InvSimpson"))
...
```{r, wilconx_hosp}
alpha_indexes_amr_family <- estimate_richness(physeq_amr_families, split = TRUE, c("Shannon", "Simpson", "InvSimpson"))
...
```{r, kruskal}
# remove NA values
ps_amr_ww_impact = subset_samples(ps_amr_good, sample_data(ps_amr_good)$Industrial_wastewater_impact != "None")
