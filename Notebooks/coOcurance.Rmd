...
Hubs are nodes in the network that have a significantly larger
number of links compared to the other nodes in the network. A
hub in a microbiome network can be considered as an equiva-
lent to a keystone species in the microbial community. Using
centrality indices (closeness and betweenness below) to find
keystone species will output a vector containing values between
0 and 1 for every node in the network. Link-analysis methods
(page_rank and hub_score below), on the other hand, will
output an object in which there is a vector containing the values
for the nodes.
